...
import { useTrialStatus } from "@/hooks/use-trial-status"; // Import the new hook
...
const AIChat: React.FC<AIChatProps> = ({ projectId, userId }) => {
  const [messages, setMessages] = useState<string[]>([]);
  const [input, setInput] = useState<string>("");
  const [loading, setLoading] = useState(false);
  const { isTrialActive, trialEndsAt, isSubscribed, loading: trialLoading } = useTrialStatus(userId); // Use the hook

  const handleSendMessage = async () => {
    if (input.trim() === "" || loading || !isTrialActive) { // Disable if trial not active
      return;
    }
    ...
    console.log(`AI Chat feature used by user ${userId} for project ${projectId} with query: "${input}"`);
  ...
  return (
    <div className="flex flex-col h-full bg-card rounded-lg shadow-lg border border-border p-4 animate-in fade-in duration-500">
      ...
      <ScrollArea className="flex-1 p-2 mb-4 border rounded-md bg-background">
        {messages.length === 0 && !trialLoading && !isFeatureRestricted ? (
        ...
        {(loading || trialLoading) && (
          <div className="flex items-center justify-center mt-4">
            ...
            <span className="text-muted-foreground text-sm">
              {trialLoading ? "Checking trial status..." : "AI is thinking..."}
            </span>
          </div>
        )}
        {isFeatureRestricted && !trialLoading && (
      ...
      <div className="flex space-x-2">
        <Input
          ...
          className="flex-1"
          disabled={loading || trialLoading || isFeatureRestricted}
        />
        <Button onClick={handleSendMessage} size="icon" disabled={loading || trialLoading || isFeatureRestricted}>
